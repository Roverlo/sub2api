...
const availablePlatformSet = computed(() => {
  if (Array.isArray(props.stats?.available_platforms)) {
    return new Set(props.stats.available_platforms.filter(Boolean))
  }
  return new Set((props.stats?.by_platform ?? []).map((item) => item.platform).filter(Boolean))
})

const hasConfiguredPlatforms = computed(() => Array.isArray(props.stats?.available_platforms))
...
const platformCards = computed<FusedPlatformCard[]>(() => {
  ...
  const platforms = availablePlatformSet.value
  ...
  if (!hasConfiguredPlatforms.value && (diffTotal > OTHER_THRESHOLD || diffToday > OTHER_THRESHOLD)) {
    cards.push({
      platform: '__other__',
