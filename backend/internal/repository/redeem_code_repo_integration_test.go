...
func (s *RedeemCodeRepoSuite) TestListWithFilters_Type() {
	...
	codes, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.RedeemCodeListFilters{Type: service.RedeemTypeSubscription})
	s.Require().NoError(err)
...
func (s *RedeemCodeRepoSuite) TestListWithFilters_Status() {
	...
	codes, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.RedeemCodeListFilters{Status: service.StatusUsed})
	s.Require().NoError(err)
	s.Require().Len(codes, 1)
...
func (s *RedeemCodeRepoSuite) TestListWithFilters_StatusExpiredByExpiresAt() {
	...
	expired, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.RedeemCodeListFilters{Status: service.StatusExpired})
	s.Require().NoError(err)
	s.Require().Len(expired, 1)
	s.Require().Equal("STAT-EXPIRED-BY-TIME", expired[0].Code)

	unused, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.RedeemCodeListFilters{Status: service.StatusUnused})
	s.Require().NoError(err)
...
func (s *RedeemCodeRepoSuite) TestListWithFilters_Search() {
	...
	codes, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.RedeemCodeListFilters{Search: "alpha"})
	s.Require().NoError(err)
	s.Require().Len(codes, 1)
	s.Require().Contains(codes[0].Code, "ALPHA")
}

func (s *RedeemCodeRepoSuite) TestListWithFilters_ValueRange() {
	...
	s.Require().NoError(s.repo.Create(s.ctx, &service.RedeemCode{Code: "VALUE-50", Type: service.RedeemTypeBalance, Value: 50, Status: service.StatusUnused}))

	minValue := 20.0
	maxValue := 20.0
	codes, _, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.RedeemCodeListFilters{
		Type:     service.RedeemTypeBalance,
		ValueMin: &minValue,
		ValueMax: &maxValue,
	})
...
func (s *RedeemCodeRepoSuite) TestCreateBatch_Filters_Use_Idempotency_ListByUser() {
	...
	list, page, err := s.repo.ListWithFilters(s.ctx, pagination.PaginationParams{Page: 1, PageSize: 10}, service.RedeemCodeListFilters{
